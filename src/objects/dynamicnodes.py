...
def __construct_nodes__(__db__):
    ...
    cconst = None
    Column = type('Column', (Node,), {'__init__':Node.__co_init__(cconst),'_construct_children':cconst})

    tconst = Node._construct_children_(Column, __db__['column'])
    Table = type('Table', (Node,), {'__init__':Node.__co_init__(tconst),'_construct_children':tconst})

    sconst = Node._construct_children_(Table, __db__['table'])
    Schema = type('Schema', (Node,), {'__init__':Node.__co_init__(sconst),'_construct_children':sconst})

    dconst = Node._construct_children_(Schema, __db__['schema'])
    Database = type('Database', (Node,), {'__init__':Node.__co_init__(dconst),'_construct_children':dconst})
